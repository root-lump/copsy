...
`base_dir` 未設定の場合、ワークツリーはメインワークツリーと同じ階層に作成されます。`flat` では `<リポジトリ名>-<ブランチ名>` という名前になり、`nested` では `<リポジトリ名>-worktrees` ディレクトリの下にまとまります。既定のクローンでは `<リポジトリ名>` がメインワークツリー自身だからです。
...
リポジトリ名はリモート `origin` の URL から取得するため、クローン先のディレクトリを改名しても変わりません。`origin` がない場合は、メインワークツリーのディレクトリ名を使います。
...
その `<リポジトリ名>` ディレクトリがメインワークツリー自身になる場合は、末尾に `-worktrees` が付きます。`git clone` はリポジトリ名のディレクトリにチェックアウトするため、`base_dir` 未設定では常にこちらになります。
